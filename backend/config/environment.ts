...
export function getEnvironmentConfig(): EnvironmentConfig {
  ...
  if (isRailway) {
    ...
    if (!process.env.MONGODB_URI) {
      errors.push(
        'MONGODB_URI is required for Railway deployment. Please add a MongoDB service or set the environment variable.'
      );
    }

    if (!process.env.JWT_SECRET) {
      errors.push(
        'JWT_SECRET is required for Railway deployment. Please set this environment variable.'
      );
    }
  ...
  if (config.NODE_ENV === 'development') {
    ...
    if (config.ADMIN_PASSWORD === 'admin123') {
      warnings.push(
        'Default admin password detected. Change ADMIN_PASSWORD for production.'
      );
    }
  }
  ...
  console.log(
    `   OPENSEA_API_KEY: ${
      config.OPENSEA_API_KEY
        ? '✅ Set'
        : '⚠️ Missing (NFT data will use mock data)'
    }`
  );
  console.log(
    `   STOCK_API_KEY: ${
      config.STOCK_API_KEY
        ? '✅ Set'
        : '⚠️ Missing (Stock data will use mock data)'
    }`
  );
